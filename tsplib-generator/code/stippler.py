...
if __name__ == '__main__':
    ...
    density = iio.imread(filename, as_gray=True, pilmode='L')
    ...
    png_filename = os.path.join(dirname, basename + "-stippled.png")
    dat_filename = os.path.join(dirname, basename + "-stippled.npy")
    tsp_filename = os.path.join(dirname, basename + ".tsp")
    ...
    if args.interactive:
        ...
        def update(frame):
            ...
            if (frame == args.n_iter-2 and
                ...
                numb_points = points.shape[0]
                # 创建 tsplib 文件
                with open(tsp_filename, 'w') as f:
                    # 写入头部信息
                    f.write(f'NAME: {tsp_filename} \n')
                    f.write(f'COMMENT: {numb_points} points\n')
                    f.write('TYPE: TSP\n')
                    f.write(f'DIMENSION: {numb_points}\n')
                    ...
                    f.write('EOF\n')
                # 成功生成提示
                print('Successfully generated tsp file!\n')
