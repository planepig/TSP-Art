...
# 图片读取
def load_image():
    global img_original, img_processed, img_label, img_out, reverse_flag
    ...
    # else:
    #     for p in point_list:
    #         img_original.putpixel(p, (0, 255, 0))
...
def threshold_adjust(val):
    ...
    if not reverse_flag:
        ...
        img_processed = img_processed.point(lambda p: 0 if p > threshold_value.get() else 255)
    img_out = img_processed
...
# Point_number控制条
Scale(root, from_=300, to=100000, orient=HORIZONTAL, label="POINTS", command=point_number_adjust).grid(row=0, column=2,
                                                                                                       padx=5, pady=5)
...
# 反相按钮
Button(root, text="REVERSE", command=reverse).grid(row=1, column=3, padx=5, pady=5)
