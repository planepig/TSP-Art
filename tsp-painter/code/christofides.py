...
def christofides(node_coords_dict):
    data = np.array(list(node_coords_dict.values()))
    # build a graph
    G = build_graph(data)
    # print("Graph: ", G)
    ...
    path = [node_coords_dict[i + 1] for i in path]
    # np.save("./output_npy/christofides.npy", np.array(path))
    return np.array(path), length